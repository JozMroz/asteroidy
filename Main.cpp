...
enum class WeaponType { LASER, BULLET, BOMB, FRAGMENT, COUNT };
class Projectile {
public:
	Projectile(Vector2 pos, Vector2 vel, int dmg, WeaponType wt, Texture2D* Texture = nullptr)
	{
	...
	void Draw() const {
		...
		if (type == WeaponType::BOMB) {
			...
			Vector2 pos = {
					transform.position.x - (texture->width * scale) / 2.5f,
					transform.position.y - (texture->height * scale) / 2.5f
			};
			DrawTextureEx(*texture, pos, 0.0f, scale, WHITE);
			}
			else {
				DrawCircleV(transform.position, 20.f, GRAY); // fallback
			}
		}
		if (type == WeaponType::FRAGMENT) {
			if (texture) {
			const float scale = 0.2f;
			Vector2 pos = {
					transform.position.x - (texture->width * scale) / 2.5f,
	...
	float GetRadius() const {
		...
		else if(type == WeaponType::LASER) return 2.f;
		else return 20.f;
	}
...
inline static Projectile MakeProjectile(WeaponType wt,
	...
	Texture2D* Texture = nullptr)
{
	Vector2 vel = Vector2Scale(Vector2Normalize(dir), speed);
	...
	else if (wt == WeaponType::FRAGMENT) {
    return Projectile(pos, vel, 10, wt, Texture); 
}
	else{
		vel = Vector2Scale(vel, 0.1f);
		return Projectile(pos, vel, 10, wt, Texture);
	}
}
...
public:
	...
	float GetVelocity(WeaponType wt) const {
		if (wt == WeaponType::LASER) return speed;
		if (wt == WeaponType::BULLET) return speed;
		else return speed;
	}
...
public:
	...
	void Run() {
		...
		InitAudioDevice();
		bombHitSound = LoadSound("bomb_hit.wav");
		bombTexture = LoadTexture("bomb.png");
		fragmentTexture = LoadTexture("frag.png");
		if (fragmentTexture.id == 0) {
    		std::cerr << "Failed to load frag.png\n";
		}
		SetTextureFilter(bombTexture, TEXTURE_FILTER_BILINEAR); // opcjonalnie
		...
		while (!WindowShouldClose()) {
			...
			// Shooting
			{
				if (player->IsAlive() && IsKeyDown(KEY_SPACE)) {
					...
					float projSpeed = player->GetSpacing(currentWeapon) * 20.f;//player->GetFireRate(currentWeapon);
					Vector2 dir = { 0, -1 };

					while (shotTimer >= interval) {
						Vector2 p = player->GetPosition();
						p.y -= player->GetRadius();
						projectiles.push_back(MakeProjectile(currentWeapon, p, dir, projSpeed, currentWeapon == WeaponType::BOMB ? &bombTexture : nullptr));
						shotTimer -= interval;
					}
				}
				else {
			...
			for (auto pit = projectiles.begin(); pit != projectiles.end();) {
				...
				for (auto ait = asteroids.begin(); ait != asteroids.end(); ++ait) {
					...
					if (dist < (*pit).GetRadius() + (*ait)->GetRadius()) {
						if (pit->GetType() == WeaponType::BOMB) {
								PlaySound(bombHitSound);
								//Texture2D* fragmentTex = &fragmentTexture;
								Vector2 pos=pit->GetPosition();
								float speed = 200.f;
								...
								for(auto& dir : directions){
									...
									projectiles.push_back(MakeProjectile(WeaponType::FRAGMENT, pos, dir, speed, &fragmentTexture));
								}
            			}
